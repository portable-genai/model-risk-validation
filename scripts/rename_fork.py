...
The catalog id (``Mrm1``) is left alone unless ``--catalog-id`` is given: it names the entry in
...
_OLD_PACKAGE = "model_risk_validation"
_OLD_ENV_PREFIX = "MRM_"
_OLD_BARE_PREFIX = "MRM"
_OLD_RESOURCE = "mrm1-svc"
_OLD_DIST = "model-risk-validation"
_OLD_CATALOG_ID = "Mrm1"
...
def _rewrite_text(text: str, args: argparse.Namespace) -> tuple[str, int]:
    ...
    # The env prefix, only where it PREFIXES an upper token (MRM_PROFILE), never mid-word.
...
def main() -> int:
    ...
    ap.add_argument(
        "--resource",
        required=True,
        help="new cloud resource stem (Terraform name_prefix), e.g. acme-mrm",
    )
