...
from model_risk_validation.domain import (
    kernel,
    models,
)
...
from model_risk_validation.domain.obligations import model_obligation_edges
...
SERVICE_NAME = "Model Risk Validation Copilot"
CATALOG_ID = "model-risk-validation"
REPOSITORY = "model-risk-validation"
...
#: A low-materiality pricing model with a full, passing validation sample: tier_3, battery
#: clear, monitoring clear, so it must NOT escalate.
ROUTINE_REQUEST = models.ValidationRequest(
...
#: A regulatory-capital IRB model: floors at tier_1 and its required tests have no samples, so
#: the run escalates and is routed for human review.
ESCALATING_REQUEST = models.ValidationRequest(
    record=InventoryRecord(
        model_id="M-IRB-001",
...
#: The same escalating path with a national id planted in the record, to prove redaction.
PII_REQUEST = models.ValidationRequest(
    record=InventoryRecord(
        model_id="M-IRB-009",
        name=(
            "Gamma IRB model (FICTIONAL), owner NRIC " + PLANTED_NRIC + ", mail ops@gamma.example"
        ),
        model_class=ModelClass.IRB,
        owner="model.owner@bank.example",
    ),
...
class DemoRun:
    ...
    def __init__(self, workdir: Path | None = None) -> None:
        ...
        self.service = ValidationService(self.container.audit, tracer=self.container.tracer)
    ...
    def _step_routine(self) -> Produced:
        return self._validation_panels(ROUTINE_REQUEST, expect_routing=False)

    def _step_escalation(self) -> Produced:
        return self._validation_panels(ESCALATING_REQUEST, expect_routing=True)

    def _step_redaction(self) -> Produced:
        panels, facts = self._validation_panels(PII_REQUEST, expect_routing=True)
    ...
    def _validation_panels(
        self, request: models.ValidationRequest, *, expect_routing: bool
    ) -> Produced:
        result = self.service.validate(request, actor=ACTOR)
        self.cases += 1
        review_ref = ""
...
def _escalating_outcome() -> models.ValidationOutcome:
    """Produce a real escalated outcome from a local run, to hand to the exit review call."""
    container = build_container(Settings(profile="local", audit_path=":memory:", tenant=TENANT))
    return ValidationService(container.audit, tracer=container.tracer).validate(
        ESCALATING_REQUEST, actor=ACTOR
    )
...
def _exit_obligations(container: Any) -> Any:
    return container.obligations.emit(
        model_obligation_edges(_escalating_outcome()),
        actor=ACTOR,
    )
