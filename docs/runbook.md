# Runbook: Model Risk Validation Copilot (`model-risk-validation`)

## Deploy (gcp)
1. `MRM_PROFILE=gcp`, install `.[gcp]`, region `asia-southeast1`.
...
3. Ingress is fronted by IAP/Apigee; the app authenticates the S2S caller fail-closed
   (`MRM_S2S_TOKEN` local, Google OIDC + allowlist secure).
4. **Set `MRM_IAP_AUDIENCE`.** Without it the service starts, stays
...
`MRM_IAP_AUDIENCE` is the IAP-protected resource the assertion must be
...
| Symptom | Cause | Fix |
|---|---|---|
| 503, detail names `MRM_IAP_AUDIENCE` | no audience configured | set it to the protected resource above |
...
| `gcp` IAP assertion | `verified` | authenticated: the signature (against IAP's own key set), the audience (against `MRM_IAP_AUDIENCE`), the expiry and the issuer are all checked before any claim is read |
...
1. **Nobody chose a profile.** `MRM_PROFILE` is absent, so no end-user
...
2. **The `local` profile, chosen deliberately.** The seeded personas are a client-asserted
   ...
   `MRM_S2S_TOKEN` is set. Setting that secret closes the S2S
...
- `add_loopback_exposure_guard` is registered at module scope in `api/app.py`, so it holds under
  `uvicorn model_risk_validation.api.app:app --host 0.0.0.0` (what the Dockerfile `CMD`
...
Set `MRM_ALLOW_INSECURE_DEMO=1` to accept the exposure deliberately.
...
`MRM_PROFILE` is read once, in `config.py`, and it has three states:
...
`MRM_S2S_TOKEN`. With routing on and the URL unset, the managed profile REFUSES TO BOOT, so a
misconfiguration is a loud failure at startup rather than at the first escalation. Under the local profile the escalation goes to the review-kit outbox, which is
inspectable and flushes to the console when one becomes reachable.

`MRM_REVIEW_ROUTING` switches routing, read in three states: unset is on, `true`/`false` (or
`on`/`off`) wins, and an emptied or unrecognised value refuses at boot. Off needs no console, logs
one warning at startup, and every validation that requires review reports `review_routing: "off"`
with an empty `review_ref`. A hand-off that fails at request time does not fail the request: the
response carries `review_routing: "failed"` and an empty reference, the failure is logged, and the
...
`MRM_AUDIT_ANCHOR` (read by `audit_anchor_path` in
`config/settings.yaml`) to a file on a DIFFERENT volume from
`MRM_AUDIT_PATH`, ideally writable by a different principal. This is
not decoration:
