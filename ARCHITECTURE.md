# ARCHITECTURE: Model Risk Validation Copilot (`model-risk-validation`)

Hexagonal ports-and-adapters. A pure-stdlib domain core speaks only to ports (`typing.Protocol`s);
adapter families implement them; one env var (`MRM_PROFILE`) swaps the
...
## Layout (`src/model_risk_validation/`)
