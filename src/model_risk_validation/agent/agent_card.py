...
class AgentCard:
    ...
    provider: str = "model-risk-validation"
...
SKILLS: tuple[AgentSkill, ...] = (
    AgentSkill(
        ...
        description=(
            "Tier one quantitative model by materiality, run its validation-test battery, record "
            "an already-redacted audit event, and ROUTE the result to human review when it "
            "escalates (rule R8). Every tier and verdict is computed by pure stdlib code, never "
            "by a model."
        ),
    ),
    AgentSkill(
...
def _resolve_url(settings: Settings) -> str:
    ...
    return f"https://model-risk-validation.{settings.region}.internal.example/a2a"
