"""FastAPI application for Model Risk Validation Copilot (model-risk-validation).
...
``MRM_PROFILE`` is NO CHOICE rather than a silent ``local``. Every
...
``MRM_S2S_TOKEN`` is wrong in the most dangerous direction:
...
header and no ``X-Dev-Persona`` then got the seeded persona list and a real validation decision.
...
from .schemas import HealthResponse, ValidationRequestModel, ValidationResponse
...
_TOKEN_ENV = "MRM_S2S_TOKEN"
_INSECURE_DEMO_ENV = "MRM_ALLOW_INSECURE_DEMO"
_CORS_ORIGINS_ENV = "MRM_CORS_ORIGINS"
_ALLOWED_CALLERS_ENV = "MRM_S2S_ALLOWED_CALLERS"
_AUDIENCE_ENV = "MRM_S2S_AUDIENCE"
_API_HOST_ENV = "MRM_API_HOST"
...
def require_service_caller(request: Request) -> None:
    ...
    stays OPEN when ``MRM_S2S_TOKEN`` is unset (loopback dev with zero
    secrets), so an UNSET ``MRM_PROFILE`` must never be allowed to
    ...
    if not _request_choice(request).service_auth_configured:
        raise HTTPException(
            ...
            detail=(
                ...
                "MRM_PROFILE is not set, so no authentication scheme "
                "has been chosen"
            ),
        )
...
_SERVICE_NAME = "model-risk-validation"
...
_APP_TITLE = "Model Risk Validation Copilot"
...
#
# Note what is NOT in this expression: MRM_S2S_TOKEN. A service
...
@app.post("/v1/validate", response_model=ValidationResponse, tags=["artifacts"])
def validate(
    request: ValidationRequestModel,
    principal: Annotated[Principal, Depends(get_principal)],
) -> ValidationResponse:
    """Validate a model; the audit actor is the verified principal, never the request body.
    ...
    result = service.validate(request.to_domain(), actor=principal.actor)
    # The hand-off never fails an already-tiered, already-audited validation; the response says
    ...
    return ValidationResponse.from_domain(
