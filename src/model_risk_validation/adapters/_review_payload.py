...
def result_to_review(result: ValidationOutcome, *, maker: str, tenant: str = "") -> Review:
    """Build the review a producer submits to human-review-console when a validation run escalates.
    ...
    return Review(
        ...
        source_key=f"model-risk-validation:{subject}:{result.severity.value}",
