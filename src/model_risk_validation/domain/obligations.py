"""Model-to-obligation records for the Rgc7 feed (slice 6), typed by obligation-register-kit.

Mrm1 does not own obligation record shapes; the shared kernel does. This module builds, from a
:class:`~.models.ValidationOutcome`, the typed graph edges Mrm1 PROPOSES into the Rgc7 register:
each says "this quantitative model answers this model-risk obligation", as an
``OBLIGATION_TO_CONTROL`` edge in the ``PROPOSED`` state. Proposed, never accepted: coverage in
Rgc7 counts only human-accepted edges, so a machine proposal from here never inflates a coverage
figure until a reviewer accepts it. Pure: it constructs kit dataclasses and performs no I/O.
...
#: firm maps these to its own Rgc7 obligation nodes. The tuple is the frozen contract this feed
...
def model_obligation_edges(outcome: ValidationOutcome) -> tuple[Edge, ...]:
    ...
    summary ride on the edge note and citation, so Rgc7 shows why the proposal was made.
    """
    control = NodeRef(NodeKind.CONTROL, outcome.subject)
    citation = KitCitation(
        source_id=f"mrm:{outcome.subject}",
        title=f"Mrm1 validation of {outcome.model_name}",
