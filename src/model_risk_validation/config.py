...
One env var (``MRM_PROFILE``) selects the adapter family for every
...
the ``${VAR:-default}`` default, SET-AND-EMPTY RAISES
:class:`~hex_service_kit.netdefaults.ConfiguredEmptyError` rather than inheriting that default or
collapsing to the empty string (an operator who emptied a value expressed an intent, and it names
...
from .ports.obligations import ObligationFeedPort
...
_PROFILE_ENV = "MRM_PROFILE"
_SETTINGS_ENV = "MRM_SETTINGS"
...
#: This service has NO generative port. The model pill says so rather than naming a stub.
_GENERATOR_PORT: str = ""
...
class ProfileChoice:
    ...
    The variable is ``MRM_PROFILE``, named once in
    :data:`_PROFILE_ENV`.
    ...
    ``os.environ.get("MRM_PROFILE", "local")``: that fallback reads an
...
DEFAULT_BINDINGS: dict[str, dict[str, str]] = {
    ...
    "obligations": {
        "local": f"{_PKG}.adapters.local.obligations:LocalObligationFeed",
        "gcp": f"{_PKG}.adapters.gcp.obligations:CloudObligationFeed",
        "onprem": f"{_PKG}.adapters.onprem.obligations:OnPremObligationFeed",
    },
}
...
def _expand(value: str) -> str:
    ...
    The rule is the adapters' rule, one layer down, so there is exactly one implementation of
    it: :func:`~.envread.setting_or_default`. UNSET takes the default written in the file,
    SET-AND-EMPTY raises rather than resolving to the empty string (which for a base URL, an
    allowlist or a path is the permissive branch), and a value wins. ``${VAR}`` with no ``:-``
    carries the empty string as its default, so unset yields empty and emptied still raises.
    """
...
def _read_settings_file(path: Path | None = None) -> dict[str, Any]:
    ...
    An EXPLICIT path (argument or ``MRM_SETTINGS``) that does not
...
REVIEW_ROUTING_ENV = "MRM_REVIEW_ROUTING"
...
class Settings:
    ...
    #: Base URL of the obligations-control-mapping register the model-to-obligation feed posts to.
    obligations_url: str = ""
...
class Container:
    ...
    @cached_property
    def obligations(self) -> ObligationFeedPort:
        adapter = self._bind("obligations")
        assert isinstance(adapter, ObligationFeedPort)
        return adapter
