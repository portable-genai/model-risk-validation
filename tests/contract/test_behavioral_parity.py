...
from model_risk_validation.adapters._review_payload import (
    result_to_review,
)
...
def test_the_payload_that_reaches_the_wire_is_redacted_whichever_family_built_it() -> None:
    ...
    result = service.validate(sample_cases.PII_REQUEST, actor=sample_cases.ACTOR)
    payload = repr(
...
def test_the_whole_pipeline_answers_on_local_and_fails_fast_on_onprem() -> None:
    ...
    result = ValidationService(local.audit, tracer=local.tracer).validate(
        sample_cases.ESCALATING_REQUEST, actor=sample_cases.ACTOR
    )
    ...
    with pytest.raises(NotImplementedError):
        ValidationService(onprem.audit, tracer=onprem.tracer).validate(
            sample_cases.ESCALATING_REQUEST, actor=sample_cases.ACTOR
        )
