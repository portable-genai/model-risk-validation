...
def _record_three(settings: Settings) -> LocalAuditAdapter:
    ...
    service = ValidationService(audit, tracer=container.tracer)
    for request in (
        sample_cases.ESCALATING_REQUEST,
        sample_cases.ROUTINE_REQUEST,
        sample_cases.PII_REQUEST,
    ):
