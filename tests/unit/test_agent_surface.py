...
from model_risk_validation.agent import (
    SKILLS,
    TOOL_FUNCTIONS,
    agent_card_document,
    build_agent_card,
...
def test_the_card_document_is_json_safe_and_names_the_region() -> None:
    ...
    assert document["name"] == "model-risk-validation"
...
def test_the_agent_path_routes_an_escalation_rather_than_only_flagging_it() -> None:
    record = sample_cases.ESCALATING_RECORD
...
def test_the_agent_path_does_not_manufacture_a_review_for_a_routine_case() -> None:
    record = sample_cases.ROUTINE_RECORD
    result = validate_model(
        record.model_id,
        record.name,
        record.model_class.value,
...
def test_the_tool_output_is_masked_before_it_can_reach_a_model() -> None:
    ...
    data.
    """
    record = sample_cases.PII_RECORD
    result = validate_model(
        record.model_id,
        record.name,
        record.model_class.value,
