...
def test_the_demo_data_is_obviously_fictional() -> None:
    requests = (demo.ROUTINE_REQUEST, demo.ESCALATING_REQUEST, demo.PII_REQUEST)
    records = [request.record for request in requests]
    for record in records:
        assert "FICTIONAL" in record.name, record.name + " does not announce itself as fake"
    blob = " ".join(record.name + " " + record.owner for record in records) + demo.ACTOR
